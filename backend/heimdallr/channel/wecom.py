...
class WecomWebhook(Channel):
    ...
    def _build_channel(self, config: dict) -> None:
        self.key = config.get(SUFFIX_WECOM_KEY, "")
        if self.key == "":
...
class WecomApp(Channel):
    ...
    def _build_channel(self, config: dict) -> None:
        self.corp_id = config.get(SUFFIX_WECOM_CORP_ID, "")
        self.secret = config.get(SUFFIX_WECOM_SECRET, "")
        agent_id_str = config.get(SUFFIX_WECOM_AGENT_ID, "")
        self.agent_id = int(agent_id_str) if agent_id_str else 0
